...
:::tip Latest Version
This page covers the full LANCommander 2.1 series. The latest patch is **2.1.16** — see [Patch Updates](#patch-updates) below for what's changed since the initial release.
:::
...
### 2.1.10
<details>
<summary>View 2.1.10 patch notes</summary>
...
### 2.1.11
<details>
<summary>View 2.1.11 patch notes</summary>
...
### 2.1.12
<details>
<summary>View 2.1.12 patch notes</summary>
...
### 2.1.13
<details>
<summary>View 2.1.13 patch notes</summary>
...
### 2.1.14
<details>
<summary>View 2.1.14 patch notes</summary>
...
### 2.1.15
<details>
<summary>View 2.1.15 patch notes</summary>
...
### 2.1.16
<details>
<summary>View 2.1.16 patch notes</summary>
...
#### Bug Fixes
- Fixed the launcher crashing when installing a tool. Install progress is now set up when the tool client is created rather than when the install plan item runs, so the UI has something to render before the install starts. Tool archives were also being downloaded, extracted, and run through their post-install script twice; that duplicate pass has been removed. Fixes [#459](https://github.com/LANCommander/LANCommander/issues/459).
- Fixed games with special characters in their name failing to add from PCGamingWiki. Titles pulled from search results are now unescaped and have underscores turned back into spaces rather than being used as the raw URL segment. Fixes [#460](https://github.com/LANCommander/LANCommander/issues/460).
- Fixed `$IPXRelayHost` and `$IPXRelayPort` never reaching scripts. The variables were only added to play actions, so install, uninstall, before start, after stop, name change, and key change scripts, along with redistributable and tool scripts, all ran without them despite the script editor advertising them. They are now populated for every script type.
- Fixed the IPX relay being ignored whenever its host was left blank, which is the normal setup for a relay hosted by the LANCommander server. A blank host now falls back to the server's address, and the relay's **Enabled** setting decides whether the variables are injected instead of whether a host happens to be filled in. Hostnames are also resolved to an IP address, since DOSBox and similar emulators can't do their own name resolution.
- Fixed `Get-UserCustomField`, `Update-UserCustomField`, and `Out-PlayerAvatar` failing to run. PowerShell constructs cmdlets itself, so the services these three expected to be injected were never supplied. They now pull what they need out of the script's session state and report failures as PowerShell errors instead of tearing down the script.
...
- Fixed archive and media files being silently dropped from an export when something else had the file open, such as the download endpoint streaming the same archive to a launcher, a backup agent, or a virus scanner. Files are now opened for shared reading, and the entry in the export is only created once the file opens, so a failure can't leave an empty file behind in the archive.
- Fixed exports including records that weren't selected and listing selected ones twice. The export started from the game's full manifest and only filled in the gaps rather than building the manifest from the queue.
...
- Fixed importing a game's custom fields writing empty records. A new field was added with no name or value attached to it, and updates to an existing field were never saved.
...
- Fixed routine launcher activity being logged as errors. Downloading your profile and deleting a cached media file were never marked as finished, so they were reported as failures.
...
<details>
<summary>View 2.1.15 downloads</summary>
...
<details>
<summary>View 2.1.14 downloads</summary>
...
<details>
<summary>View 2.1.13 downloads</summary>
...
<details>
<summary>View 2.1.12 downloads</summary>
...
<details>
<summary>View 2.1.11 downloads</summary>
...
<details>
<summary>View 2.1.10 downloads</summary>
...
<details>
<summary>View 2.1.9 downloads</summary>
...
<ContributorGrid from="v2.0.2" to="v2.1.16" />
